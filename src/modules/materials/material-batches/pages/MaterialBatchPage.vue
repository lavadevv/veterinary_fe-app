              ...
              <div class="bg-white rounded-lg border border-gray-200 p-3 sm:p-4 shadow-sm hover:shadow-md transition-shadow duration-200">
                <div class="flex items-center">
                  ...
                  <div class="ml-3 min-w-0 flex-1">
                    ...
                    <p class="text-sm sm:text-base font-semibold text-gray-900">{{ totalMaterialBatches }}</p>
                  </div>
                </div>
              </div>
...
const readyBatches = computed(() => {
  return materialBatches.value?.filter(batch => batch.status === 'READY').length || 0
})

const quarantineBatches = computed(() => {
  return materialBatches.value?.filter(batch => batch.status === 'QUARANTINE').length || 0
})

const expiredSoonBatches = computed(() => {
  if (!materialBatches.value) return 0
  const now = new Date()
  const daysThreshold = 30 // 30 days warning
  
  return materialBatches.value.filter(batch => {
...
const handlePageSizeUpdate = (pageSize) => {
  materialBatchStore.setPageSize(pageSize)
}

const handleClearFilters = () => {
  materialBatchStore.resetFilters()
}
...
const handleSaveQuantity = async (quantityData) => {
  try {
    await materialBatchStore.updateMaterialBatchQuantity(selectedBatch.value.id, quantityData.quantity)
...
const handleSaveLocation = async (locationData) => {
  try {
    // TODO: Implement move location functionality
    // await materialBatchStore.updateMaterialBatch(selectedBatch.value.id, { locationId: locationData.locationId })
    console.log('Move location not implemented yet:', locationData)
...
const handleSaveStatus = async (statusData) => {
  try {
    if (statusData.type === 'test') {
      await materialBatchStore.updateMaterialBatchTestStatus(selectedBatch.value.id, statusData.status)
    } else {
      await materialBatchStore.updateMaterialBatchUsageStatus(selectedBatch.value.id, statusData.status)
    }
...
watch(() => router.currentRoute.value.name, (newName) => {
  if (newName === 'MaterialBatchList') {
    refreshData()
  }
})

// Watch for filter changes to automatically fetch data
